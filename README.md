...
- `mnist_cnn_best.pth`：训练过程中保存的最佳模型权重（测试准确率最高）。
- `mnist_cnn_model.pth`：最终训练结束后的模型权重。
- `README.md`：项目说明文档。

## 扩展建议

- 调整网络深度或添加数据增强可进一步提升准确率。
- 可使用 LeNet-5 或 ResNet 等经典模型进行对比实验。
- 部署为简单的 Web API 或 GUI 应用以进行实时手写数字识别。
